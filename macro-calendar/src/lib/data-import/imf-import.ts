/**
 * IMF Historical Data Bulk Import Script
 *
 * This script imports historical economic data from the IMF World Economic Outlook (WEO)
 ...
 *   - IMF_IMPORT_START_YEAR: Start year for import (default: 2014, 10+ years)
 *   - IMF_IMPORT_END_YEAR: End year for import (default: current year)
 *   - IMF_IMPORT_INDICATORS: Comma-separated list of indicator IDs to import
 *   - IMF_IMPORT_COUNTRIES: Comma-separated list of country codes to import
 *
 * Features:
 *   - Imports historical data for all configured IMF indicators
 *   - Deduplicates data using (indicator_id, release_at, period) key
 *   - Handles rate limiting (1 second between requests for fair use)
...
// Source information
const IMF_SOURCE_NAME = "IMF World Economic Outlook";
const IMF_SOURCE_URL = "https://www.imf.org/en/Publications/WEO";
...
/**
 * Get or create an indicator for an IMF indicator + country combination.
 */
async function getOrCreateIndicator(
  supabase: SupabaseClient,
  imfIndicatorId: IMFIndicatorId,
  countryCode: IMFCountryCode
): Promise<string> {
  const config = IMF_INDICATOR_CONFIG[imfIndicatorId];
  const countryName = IMF_COUNTRIES[countryCode];
  ...
  const { data: inserted, error: insertError } = await supabase
    ...
    .insert({
      ...
      source_name: IMF_SOURCE_NAME,
      source_url: IMF_SOURCE_URL,
    })
    .select("id")
    .single();
...
async function importIndicatorObservations(
  ...
  // Convert IMF observations to ObservationData format for validation
...
/**
 * Import data for a single IMF indicator across all countries.
 */
async function importIndicator(
  imfClient: IMFClient,
  supabase: SupabaseClient,
  indicatorId: IMFIndicatorId,
  countryCodes: IMFCountryCode[],
  startYear: string,
  endYear: string,
  validationOptions: ValidationOptions = {}
): Promise<IndicatorCountryImportResult[]> {
  const config = IMF_INDICATOR_CONFIG[indicatorId];
  const results: IndicatorCountryImportResult[] = [];

  try {
    console.log(`  Fetching ${indicatorId}: ${config.name} for ${countryCodes.length} countries`);

    // Fetch all observations for all countries
    const observations = await imfClient.getIndicatorObservations(
      indicatorId,
      countryCodes,
      startYear,
      endYear
    ...
    const observationsByCountry = new Map<string, IMFObservation[]>();
 ...
 */
export async function importIMFHistoricalData(
  options: {
    startYear?: string;
    endYear?: string;
    indicatorIds?: IMFIndicatorId[];
    countryCodes?: IMFCountryCode[];
  ...
  const indicatorIds =
    options.indicatorIds ?? (Object.keys(IMF_INDICATOR_CONFIG) as IMFIndicatorId[]);
  const countryCodes =
    options.countryCodes ?? (Object.keys(IMF_COUNTRIES) as IMFCountryCode[]);
  const validationOptions: ValidationOptions = options.validation ?? {};

  console.log("=".repeat(60));
  console.log("IMF Historical Data Import");
  console.log("=".repeat(60));
...
if (require.main === module) {
  const startYear = process.env.IMF_IMPORT_START_YEAR ?? DEFAULT_START_YEAR;
  const endYear = process.env.IMF_IMPORT_END_YEAR ?? new Date().getFullYear().toString();
  
  const indicatorsEnv = process.env.IMF_IMPORT_INDICATORS;
  const indicatorIds = indicatorsEnv
    ? (indicatorsEnv.split(",").map((s) => s.trim()) as IMFIndicatorId[])
    : undefined;

  const countriesEnv = process.env.IMF_IMPORT_COUNTRIES;
  const countryCodes = countriesEnv
    ? (countriesEnv.split(",").map((s) => s.trim()) as IMFCountryCode[])
    : undefined;

  importIMFHistoricalData({ startYear, endYear, indicatorIds, countryCodes })
    .then((result) => {
