...
/**
 * IMFImportButton component for admin dashboard.
 * Displays IMF API status and allows triggering data imports.
 * IMF World Economic Outlook API is free and open - no API key required.
 */
export function IMFImportButton() {
  const [status, setStatus] = useState<IMFImportStatus | null>(null);
  ...
  // Fetch IMF configuration status on mount
  useEffect(() => {
    async function fetchStatus() {
      try {
        const response = await fetch("/api/admin/imf-import");
        if (response.ok) {
          const data = await response.json();
          setStatus(data);
        } else {
          const errorData = await response.json();
          setError(errorData.error || "Failed to fetch IMF status");
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to fetch IMF status");
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400">
              Import global economic data from IMF World Economic Outlook
            </p>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="p-4">
        ...
        <div>
          <p className="mb-4 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            GDP growth, inflation, unemployment, trade balance, and government debt from the IMF World Economic Outlook.
          </p>
          <button
