...
### Added
- **T491:** Mobile watchlist and alerts screens — `WatchlistScreen` now fetches the signed-in user's watchlist from Supabase (indicator name, next release time, tap-to-detail stub) with pull-to-refresh and an auth gate for unauthenticated users. New `AlertsScreen` shows upcoming releases in the next 7 days for watchlisted indicators, grouped by day with impact badges, pull-to-refresh, and auth gate. `AlertsScreen` added as a fourth tab in the bottom navigator. New `mobile/src/lib/supabase.ts` Supabase client; `fetchWatchlist` and `fetchWatchlistAlerts` helpers added to `mobile/src/lib/api.ts`.
