...
### Added
- **T510:** Embeddable calendar widget — `/widget/calendar` renders a stripped-down, auth-free calendar showing the next 7 days of economic releases; supports query params `?countries=US,EU&impact=high&theme=dark|light`. `/api/v1/embed/code` (public, no auth) returns the iframe HTML snippet as JSON. `/settings/embed` lets authenticated users configure widget options (countries, impact, theme, height) with a live preview and one-click copy of the embed snippet. Widget pages skip the global header, usage banner, and session-refresh middleware.
