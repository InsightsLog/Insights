...
### Added
- **T512:** Mobile push notifications — the Insights mobile app now requests push permission on launch, registers the Expo push token, and persists it to the `push_subscriptions` table (`token_type='expo'`). Tapping a notification navigates to the Calendar screen. The `send-push-notification` edge function now sends via the Expo Push API for expo-type subscriptions in addition to the existing Web Push Protocol path. New migration 029 adds `token_type` and `expo_token` columns to `push_subscriptions` and makes `endpoint`/`keys` nullable for expo-type rows. New `mobile/src/lib/notifications.ts` contains all token registration helpers.
- **T500:** Slack webhook integration at `/settings/integrations` — users can connect a Slack incoming webhook URL (must start with `https://hooks.slack.com/`) to receive release alerts in their Slack workspace. Includes a "Send Test Message" button that posts a sample formatted message. The `send-webhook` edge function now detects Slack endpoints and delivers Slack-formatted payloads (`{ text, attachments }`). New migration 028 adds a `type` column to `webhook_endpoints`.
